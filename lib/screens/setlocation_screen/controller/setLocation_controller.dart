...
import 'package:food_delivery_app/model/UserInfo.dart';
...
class SetLocationController extends GetxController {
  ...
  final RxList<Marker> markers = <Marker>[
    const Marker(
        ...
        infoWindow: InfoWindow(title: 'Place marker'))
  ].obs;
  ...
  saveLocation(context) async {
    ...
    try {
      if (address.value != 'Your Current Location') {
        ...
        print('here we add user');
        print('heyyyy  ${userRef.child('$uid/firstN').get()}');

        UserDetails userDetails = UserDetails();
        //Get the uid needed
        final chatUserDetailsRef = userRef.child('$uid/firstN').get();
        //define user object

        // final chatUserRef = userRef.child(uid);
        // chatUserRef.get().then((DataSnapshot snapshot) {
        ...
        var ref2 = await userRef.child(uid).once();

        // ref2.once().then((snapshot) {
        //   print(snapshot.snapshot.value);
        //   UserDetails? userDetails = UserDetails();
        //   userDetails = snapshot.snapshot.value as UserDetails?;
        //
        //   print(snapshot.snapshot.key);
        //   snapshot.snapshot.children.forEach((elements) {
        //     print(elements.value);
        //   });
        // });

        // chatUserDetailsRef.then((value) => print(value.value) );
        // await Services.cloudStore.collection('chats').add(
        //   {
        //     'name' :  chatUserDetailsRef.then((value) => value.value),
        //     'uid' : uid,
        //   }
        // );
